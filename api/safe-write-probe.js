export default async function handler(req,res){
  ...
  try{
    ...
    if(!data){
      ...
      return res.status(502).json({ok:false,error:'upstream_invalid_json',upstreamStatus:upstream.status,contentType:String(upstream.headers.get('content-type')||''),finalHost:(()=>{try{return new URL(upstream.url).host}catch{return''}})(),baseDeploymentId,detail:detail||decoded.slice(0,500)});
    }
    return res.status(data.ok===true?200:502).json({ok:data.ok===true,status:String(data.status||''),applied:!!data.applied,requestId:String(data.requestId||''),ack:String(data.ack||''),error:String(data.error||''),baseDeploymentId});
  }catch(e){return res.status(502).json({ok:false,error:String(e&&e.message?e.message:e),baseDeploymentId});}
}
