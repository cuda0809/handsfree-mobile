// HandsFree Mobile REAL 0.7 — Shadow Mode parity + canonical + decision gate.
...
const {buildDecisions}=require('../../core/decision-v07');
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const validRisks=new Set(['긴급','주의','협의','안정']);
    const decisionErrors=[];
    if(decision.projects.length!==canonical.projectMaster.filter(p=>p.active).length)decisionErrors.push('decision active project count mismatch');
    for(const d of decision.projects){
      if(!validRisks.has(d.risk))decisionErrors.push(`${d.projectId}: invalid risk`);
      if(!text(d.nextGate))decisionErrors.push(`${d.projectId}: next gate missing`);
      if(dateOnly(d.due)&&d.dueDays!==dayDiff(d.due,decision.asOf))decisionErrors.push(`${d.projectId}: D-Day mismatch`);
      if(d.blocked&&!d.blockType)decisionErrors.push(`${d.projectId}: blocked without block type`);
    }
    if(decisionErrors.length)criticalErrors.push(...decisionErrors.map(error=>({type:'decision',error})));
    ...
    if(progressWarnings.length)warnings.push(`${progressWarnings.length} projects have plan-progress vs actual-progress differences; canonical model keeps them separate`);
    if(canonical.quality.orphanEvents.length)warnings.push(`${canonical.quality.orphanEvents.length} historical/support events are not linked to current master projects`);
    if(decision.metrics.decisions>0)warnings.push(`${decision.metrics.decisions} projects require manager decision/review today`);

    const sampleSize=projectChecks.length+eventChecks.length+issueChecks.length,shadowReady=criticalErrors.length===0,promotionReady=shadowReady&&warnings.filter(w=>!w.includes('manager decision')).length===0;
    return res.status(shadowReady?200:409).json({
      ...
      canonical:{schema:canonical.schema,projectMaster:canonical.projectMaster.length,activeProjects:canonical.projectMaster.filter(p=>p.active).length,eventLog:canonical.eventLog.length,duplicateEventIds:canonical.quality.duplicateEventIds.length,orphanEvents:canonical.quality.orphanEvents.length,progressConflicts:canonical.quality.progressConflicts.length},
      decision:{schema:decision.schema,asOf:decision.asOf,metrics:decision.metrics,errors:decisionErrors,sample:decision.projects.slice(0,12)},
      errors:{count:criticalErrors.length,projects:projectErrors,events:eventShapeErrors,issues:issueErrors,decision:decisionErrors},warnings,progressWarnings:progressWarnings.slice(0,10),samples:{projects:projectChecks,events:eventChecks,issues:issueChecks}
    });
