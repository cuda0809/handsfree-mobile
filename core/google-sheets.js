// HandsFree Mobile REAL 0.3 — authenticated Google Sheets read bridge.
...
function buildSnapshot(valueRanges){
  const [productVR,workVR,issueVR,changeVR,capaVR]=valueRanges,productRows=rows(productVR),workRows=rows(workVR),issueRows=rows(issueVR),changeRows=rows(changeVR),capaRows=rows(capaVR);
  ...
  const issues=issueRows.slice(1).filter(r=>r[0]).map(r=>({id:r[0],openedAt:serialDate(r[1]),type:r[2],projectId:r[3],customer:r[4]??null,model:r[5]??null,process:r[6]??null,sourceEventId:r[7]??null,status:String(r[8]??'OPEN').toUpperCase(),latestUpdate:serialDate(r[9]),resolution:r[10]??null,closedAt:serialDate(r[11]),sourceRow:r[12]??null,briefingActive:String(r[13]??'').toUpperCase()==='TRUE',searchKey:r[14]??null,note:r[15]??null}));
  const changes=changeRows.slice(1).filter(r=>r[0]||r[3]).map(r=>({date:serialDate(r[0]),projectId:r[3]??null,customer:r[4]??null,model:r[5]??null,process:r[6]??null,initialPlan:serialDate(r[7]),previousDate:serialDate(r[8]),effectiveDate:serialDate(r[9]),deltaDays:num(r[10]),cumulativeDays:num(r[11]),extendedDays:num(r[12]),shortenedDays:num(r[13]),changeType:r[14]??null,reason:r[15]??null,summary:r[15]??null,raw:r[16]??null,sourceRow:r[17]??null,current:String(r[18]??'').toUpperCase()==='TRUE',searchKey:r[19]??null,source:'HF_DATA_일정변경누적'}));
  const capacity=capaRows.slice(1).filter(r=>r[0]||r[1]).map(r=>{const available=num(r[12]),demand=num(r[13]);return{date:serialDate(r[0]),projectId:r[1]??null,customer:r[2]??null,model:r[3]??null,due:serialDate(r[4]),process:r[5]??null,masterCrew:r[6]??null,recentWorkers:r[7]??null,crewSource:r[8]??null,crewFTE:num(r[9]),absent:r[10]??null,directLossFTE:num(r[11]),availableFTE:available,demandFTE:demand,shortageFTE:num(r[14]),gap:available-demand,impactDays:num(r[15]),risk:r[16]??null,summary:r[17]??null,decisionRequired:/HIGH|OVERLOAD/.test(String(r[16]??'')),source:'HF_VIEW_인력CAPA'};});
  return{meta:{asOf:kstToday(),generatedAt:new Date().toISOString(),source:'GOOGLE_SHEETS_LIVE'},projects,events,issues,changes,capacity};
}
async function loadSnapshot(){if(!configured())throw new Error('Google Sheets bridge credentials are not configured');const ranges=["'제품마스터'!A4:L1100","'업무이력'!A7:I220","'HF_DATA_이슈원장'!A1:P1000","'HF_DATA_일정변경누적'!A1:T2000","'HF_VIEW_인력CAPA'!A1:R2000"];const valueRanges=await batchGet(ranges);if(valueRanges.length<ranges.length)throw new Error(`Google Sheets returned ${valueRanges.length}/${ranges.length} ranges`);return buildSnapshot(valueRanges);}
