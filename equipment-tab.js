(()=>{
  ...
  function applyEquipment(){
    equipmentMode=true;
    const rows=(window.STATE&&Array.isArray(STATE.data))?STATE.data:[];
    ...
    const sync=$('#syncText'); if(sync) sync.textContent=(window.live?'LIVE':'SNAPSHOT')+' · 장비 통합조회';
    ...
    const focusLabel=$('.focus small'); if(focusLabel) focusLabel.textContent='장비 빠른 확인';
    const focusText=$('#focusText'); if(focusText) focusText.textContent='검색하거나 장비 카드를 선택해 상세 상태를 확인';
    const focusBtn=$('#focusBtn'); if(focusBtn){focusBtn.textContent='전체보기';focusBtn.onclick=()=>{if(search){search.value='';search.blur()}window.FILTER='all'; if(typeof window.render==='function') window.render();};}
  }
